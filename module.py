...
def hypothesis_testing(data,col,test_value, threshold, n_sample, two_tailed):
    ...
    if two_tailed:
        ...
        if z_score>z_critical:
            print(f'Hypothesis rejected as the z score is {(test_value-mean)/std} which is more than the z critical {norm.ppf(threshold)}')
        else:
            print(f'Hypothesis accpeted as the z score is {(test_value-mean)/std} which is less than the z critical {norm.ppf(threshold)}')
    else:
        z_critical = abs(norm.ppf(threshold))
        if z_score>z_critical:
            print(f'Hypothesis rejected as the z score is {(test_value-mean)/std} which is more than the z critical {norm.ppf(threshold)}')
